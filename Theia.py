...
translation_button_label = ttk.Label(buttons_frame, text="Translation").grid(row=4, column=1, columnspan = 2)
...
wrap_around_check.grid(row=7, column=1, columnspan=2)

translation_button = ttk.Button(buttons_frame, text="Translate", command=lambda: translate(canvas, int(delta_x_entry.get()),
                                                                                           int(delta_y_entry.get()), wrap_around.get()))
translation_button.grid(row=8, column=1, columnspan=2, sticky=W+E, pady=5)

ttk.Separator(buttons_frame, orient=HORIZONTAL).grid(row=9, columnspan=3, sticky=(W, E),pady=8)
